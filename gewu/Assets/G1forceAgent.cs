...
public class G1forceAgent : Agent
{
    ...
    void RecordVxData()
    {
        ...
        var acceleration = (arts[0].velocity - lastVelocity) / deltaTime;
        lastVelocity = arts[0].velocity;
 
        // 添加数据点（X:时间, Y:vx值）
        lineChart.AddData(0, currentTime, arts[0].velocity.magnitude);
 
        // 可选：限制数据点数量（例如最多100个点）
        if (lineChart.series[0].dataCount > 200)
        {
            //neChart.RemoveData(0); // 移除最早的数据点
        }
    }
    ...
    void Start()
    {
        ...
        if (!_isClone) 
        {
            for (int i = 1; i < numrob; i++)
            {
                ...
                clone.GetComponent<G1forceAgent>()._isClone = true; 
            }
        }
    }
    ...
    public override void OnEpisodeBegin()
    {
        ...
        if (!fixbody)
        {
            ...
            if(!train)arts[0].TeleportRoot(pos0, rot0);
    ...
    void FixedUpdate()
    {
        ...
        if(tt==800 && !train)arts[0].AddForce(Force, ForceMode.Impulse);
        ...
        var vel_reward2 = 0*Mathf.Clamp(vel[2],-2,1.2f) - Mathf.Abs(vel[2]) - Mathf.Abs(vel[0]) + kh * Mathf.Abs(vel[1]);
